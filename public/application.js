...
var config = {
  apiOrigins: {
    ...
    production: 'https://fill-ups.herokuapp.com'
  }
};
...
var getFillUps = function getFillUps() {
  api.getFillUps().then(function (data) {
    ui.getFillUpsSuccess(data);
    addRowHandlers();
  }).catch(ui.failure);
};
var onAddButton = function onAddButton(event) {
  event.preventDefault();
  ui.displayAddForm();
};
var onAddFillUp = function onAddFillUp(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.addFillUp(data).then(ui.addFillUpSuccess).then(getFillUps).catch(ui.failure);
};
var onDeleteFillUp = function onDeleteFillUp(event) {
  event.preventDefault();
  var index = $(event.target).attr('data-id');
  api.deleteFillUp(index).then(function () {
    ui.deleteFillUpSuccess(index);
  }).then(getFillUps).catch(ui.failure);
};
var triggerEditForm = function triggerEditForm() {
...
var onUpdate = function onUpdate(event) {
  ...
  .then(getFillUps).then(ui.updateFillUpSuccess).catch(ui.failure);
};
var clearLoginForm = function clearLoginForm() {
  $('#login')[0].reset();
};
var clearSignUpForm = function clearSignUpForm() {
  $('#sign-up')[0].reset();
};
var clearChangePassForm = function clearChangePassForm() {
...
var addRowHandlers = function addRowHandlers() {
  $('.deleteFillUpButton').on('click', onDeleteFillUp);
  $('.editFillUpButton').on('click', triggerEditForm);
};
var addUpdateHandlers = function addUpdateHandlers() {
...
var showFillUpsTemplate = __webpack_require__(19);

var clearFillUps = function clearFillUps() {
  $('.content').empty();
};
var sortRevChron = function sortRevChron(data) {
  data.fill_ups.sort(function (a, b) {
    return Date.parse(b.date) - Date.parse(a.date);
  });
};

var calculateMPG = function calculateMPG(data) {
  data.fill_ups.forEach(function (obj, index, arr) {
    if (index === arr.length - 1) {
      obj.mpg = (obj.mileage / obj.gallons).toFixed(1);
    } else {
      var miles = obj.mileage - data.fill_ups[index + 1].mileage;
      obj.mpg = (miles / obj.gallons).toFixed(1);
    }
  });
};

var calculateAllTimeTotal = function calculateAllTimeTotal(data) {
  data.fill_ups.reduce(function (total, val) {
    var sum = total + val.price;
...
var calculateYTDTotal = function calculateYTDTotal(data) {
  var sum = 0;
  var yearArray = [];
  data.fill_ups.forEach(function (obj, index) {
    var date = data.fill_ups[index].date;
...
var calculateMonthlyAvg = function calculateMonthlyAvg(data) {
  data.fill_ups.forEach(function (obj) {
    var mostRecentDate = data.fill_ups[0].date;
    var mostRecentMonth = mostRecentDate.substr(5, 2);
    var avg = calculateYTDTotal(data) / +mostRecentMonth;
...
var getFillUpsSuccess = function getFillUpsSuccess(data) {
  clearFillUps();
  sortRevChron(data);
  calculateMPG(data);
  calculateAllTimeTotal(data);
  calculateMonthlyAvg(data);
  var showFillUpsHtml = showFillUpsTemplate({ fill_ups: data.fill_ups });
...
var displayAddForm = function displayAddForm() {
  $('#add-button').hide();
  $('.update-form').hide();
  $('.content').hide();
  $('.total-spent').hide();
...
var addFillUpSuccess = function addFillUpSuccess(data) {
  $('#add-fill-up')[0].reset();
  $('.add-fill-up').hide();
  $('#add-button').show();
  $('.total-spent').show();
...
var deleteFillUpSuccess = function deleteFillUpSuccess(index) {
  clearFillUps();
  $('.add-fill-up').hide();
  $('#add-button').show();
};

var triggerEditForm = function triggerEditForm(event) {
  $('#add-button').hide();
  $('.add-fill-up').hide();
  $('.content').hide();
  $('.total-spent').hide();
  ...
  var date = tr.find('td.fill_up-date').text();
  var dateField = $('#update').find('input[name="fill_up[date]"]');
  dateField.attr('value', date);

  var mileage = tr.find('td.fill_up-mileage').text();
  var mileageField = $('#update').find('input[name="fill_up[mileage]"]');
  mileageField.attr('value', mileage);

  var gallons = tr.find('td.fill_up-gallons').text();
  var gallonsField = $('#update').find('input[name="fill_up[gallons]"]');
  gallonsField.attr('value', gallons);

  var price = tr.find('td.fill_up-price').text();
  var priceField = $('#update').find('input[name="fill_up[price]"]');
  priceField.attr('value', price);
  //
  var id = $(event.target).attr('data-id');
  var idField = $('#update').find('input[name="fill_up[id]"]');
  idField.attr('value', id);
};

var updateFillUpSuccess = function updateFillUpSuccess() {
  $('#update')[0].reset();
  $('.update-form').hide();
  $('#add-button').show();
  $('.total-spent').show();
...
module.exports = {
  getFillUpsSuccess: getFillUpsSuccess,
  clearFillUps: clearFillUps,
  displayAddForm: displayAddForm,
  addFillUpSuccess: addFillUpSuccess,
  deleteFillUpSuccess: deleteFillUpSuccess,
  triggerEditForm: triggerEditForm,
  updateFillUpSuccess: updateFillUpSuccess,
  failure: failure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var fillUpsEvents = __webpack_require__(7);

$(function () {
  setAPIOrigin(location, config);
  authEvents.addHandlers();
  fillUpsEvents.addHandlers();
});
...
var onSignUp = function onSignUp(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.signUp(data).then(api.signUpSignIn).then(ui.signInSuccess).catch(ui.signUpFailure);
};
...
var clearSignUpMessage = function clearSignUpMessage() {
  $('#signup-message').text('');
};
...
var clearLoginMessage = function clearLoginMessage() {
  $('#login-message').text('');
};

var clearPassForm = function clearPassForm() {
  $('#change-password')[0].reset();
};

var clearPassMessage = function clearPassMessage() {
...
var fillUpsEvents = __webpack_require__(7);
var fillUpsUi = __webpack_require__(8);
...
var addFillUp = function addFillUp(data) {
  return $.ajax({
    url: config.apiOrigin + '/fill_ups',
    method: 'POST',
    headers: {
...
var deleteFillUp = function deleteFillUp(data) {
  return $.ajax({
...
var updateFillUp = function updateFillUp(data) {
  return $.ajax({
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "\n      <tr data-id=\""
    ...
    + "\">\n        <td class = \"fill_up-date\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.date : stack1), depth0))
    + "</td>\n        <td class = \"fill_up-mileage\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.mileage : stack1), depth0))
    + "</td>\n        <td class = \"fill_up-gallons\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.gallons : stack1), depth0))
    + "</td>\n        <td class = \"fill_up-mpg\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.mpg : stack1), depth0))
    + "</td>\n        <td class = \"fill_up-price\">"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.price : stack1), depth0))
    + "</td>\n        <td>\n          <button data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"editFillUpButton btn btn-info\">Edit</button>\n          <button data-id=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"deleteFillUpButton btn btn-danger\">Delete</button>\n        <td>\n      </tr>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<table class=\"table table-responsive table-condensed table-bordered\">\n  <h4 class=\"table-title\">Recent Fill-Ups</h4>\n  <thead>\n    <tr>\n      <th class=\"th\">Date</th>\n      <th class=\"th\">Mileage</th>\n      <th class=\"th\">Gallons</th>\n      <th class=\"th\">MPG since last fill-up</th>\n      <th class=\"th\">Price</th>\n      <th class=\"th\">Actions</th>\n    </tr>\n  </thead>\n  <tbody>\n\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.fill_ups : depth0),{"name":"each","hash":{},"fn":container.program(1, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </tbody>\n</table>\n";
